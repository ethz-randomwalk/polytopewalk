...
In ``PolytopeWalk``, we implement 4 interior-point-method-based MCMC sampling algorithms and 2 standard random walk algorithms (6 total) in both the sparse-constrained and full-dimensional formulation. ``PolytopeWalk`` makes meaningful strides in the open-source development of MCMC, speeding up calculations for sparse high-dimensional sampling. Finally, we provide an open-source implementation of the Facial Reduction algorithm, described in detail in the Preprocessing Algorithms section. Additional technical details can be found in a separate paper [@sun2024polytopewalksparsemcmcsampling].
